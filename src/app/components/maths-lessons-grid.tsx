...
import { BookOpen, Lock, CheckCircle2, Star, Trophy, Calculator, ArrowLeft } from "lucide-react";
import { Button } from "./ui/button";
import { Badge } from "./ui/badge";
import { useMemo } from "react";
import { getMathsLessons, getMathsProgress, isMathsLessonUnlocked } from "../../services/localStorage";
import { useLanguage } from "../../contexts/LanguageContext";
import { MATHS_UNIT_NAMES } from "../../services/maths-lessons";
import type { Lesson } from "../../services/types";

interface MathsLessonsGridProps {
  onLessonClick: (lessonId: number) => void;
  onBack?: () => void;
}

export function MathsLessonsGrid({ onLessonClick, onBack }: MathsLessonsGridProps) {
  const { language } = useLanguage(); // triggers re-render on language switch
  const lessons = getMathsLessons();
  const progressData = getMathsProgress();
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-white to-gray-50 dark:from-gray-950 dark:to-gray-900" id="maths-lessons">
      <div className="container mx-auto px-4">
        ...
        {onBack && (
          <div className="mb-6">
            <Button
              variant="ghost"
              onClick={onBack}
              className="flex items-center gap-2 text-gray-600 hover:text-purple-700 hover:bg-purple-50"
            >
        ...
        >
          <div className="inline-flex items-center gap-2 bg-purple-100 text-purple-700 px-4 py-2 rounded-full text-sm font-bold mb-4">
            ...
            Sri Lankan O/L Mathematics
          </div>
          <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2">
            All Mathematics Lessons
          </h2>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Grade 10-11 O/L Mathematics Syllabus — Number Systems to Trigonometry
          </p>
        </motion.div>
        ...
        <div className="max-w-6xl mx-auto space-y-12">
          {Object.entries(lessonsByUnit).map(([level, unitLessons]) => {
            ...
            const unitInfo = MATHS_UNIT_NAMES[levelNum] || { name: `Unit ${level}`, color: "bg-gray-500", gradient: "from-gray-400 to-gray-600" };
              ...
              <div key={level}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {unitLessons.map((lesson, index) => {
                    ...
                    const isUnlocked = isMathsLessonUnlocked(lesson.id);
                        ...
                        <div className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden transition-all group ${isUnlocked ? 'hover:shadow-xl' : 'opacity-75'}`}>
                          <div className={`relative h-32 bg-gradient-to-br ${unitInfo.gradient} flex items-center justify-center p-4 ${!isUnlocked ? 'opacity-50' : ''}`}>
                            <div className="absolute top-3 left-3 bg-white/90 dark:bg-gray-700/90 backdrop-blur-sm w-10 h-10 rounded-full flex items-center justify-center shadow-md">
                              <span className="text-sm font-bold text-gray-900">{lesson.id - 1000}</span>
                            </div>
                          ...
                          <div className="p-5">
                            <h4 className={`text-lg font-bold mb-2 line-clamp-2 min-h-[3.5rem] ${isUnlocked ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-500'}`}>
                              {lesson.title}
                            </h4>
                            <p className={`text-sm mb-4 line-clamp-2 min-h-[2.5rem] ${isUnlocked ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`}>
